...
from ..common.performance import get_performance_metrics, perf_stats
...
def run_policy_through_env_realdata(env, model, device, deterministic=True):
    ...
    while not done:
        ...
        w = env._action_to_weights_with_BIL_residual(a_np, bil_idx=env.bil_idx, allow_short_risky=True)
    ...
    z_df = pd.DataFrame(Z, columns=[f"z{j}" for j in range(z_dim)])
...
def build_agent_regime_outputs(out, df):
