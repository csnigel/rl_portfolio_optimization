...
from rl_portfolio_optimization.notebook_v1_gaussian import (
    BENCHMARK,
    PortfolioEnvV1,
...
def main() -> None:
    repo_root = REPO_ROOT
    report = RunReport.create(repo_root / "reports", "notebook_v1_gaussian")
    ...
    train_env = PortfolioEnvV1(panel, DEFAULT_TICKERS, DEFAULT_ASSET_COLUMNS, DEFAULT_SHARED_COLUMNS, DEFAULT_MACRO_COLUMNS, tc_bps=0, l2_gamma=0, start_date="2016-12-31", end_date="2022-12-31")
    val_env = PortfolioEnvV1(panel, DEFAULT_TICKERS, DEFAULT_ASSET_COLUMNS, DEFAULT_SHARED_COLUMNS, DEFAULT_MACRO_COLUMNS, tc_bps=0, l2_gamma=0, start_date="2023-01-31", end_date="2023-12-31")
    test_env = PortfolioEnvV1(panel, DEFAULT_TICKERS, DEFAULT_ASSET_COLUMNS, DEFAULT_SHARED_COLUMNS, DEFAULT_MACRO_COLUMNS, tc_bps=0, l2_gamma=0, start_date="2024-01-31", end_date="2025-12-31")
    complete_env = PortfolioEnvV1(panel, DEFAULT_TICKERS, DEFAULT_ASSET_COLUMNS, DEFAULT_SHARED_COLUMNS, DEFAULT_MACRO_COLUMNS, tc_bps=0, l2_gamma=0, start_date="2016-12-31", end_date="2025-12-31")

    bundle = train_gaussian_v1(train_env, val_env)
    model = bundle.model
    ...
    test_roll = bundle.model  # placeholder to keep explicit structure similar to notebook narrative

    out_test = run_policy_through_env_realdata(test_env, model, device=device, deterministic=True)
    fig = plt.figure()
    ...
    mc_df = mc_test_policy_noise(test_env, model, device=device, num_sims=1000)
    report.save_dataframe(mc_df, "mc_metrics.csv")
    fig = plot_metrics_dashboard(mc_df, BENCHMARK, "V1 Agent Perfomance Distribution v/s Benchmark Dashboard")
    ...
    plt.show()
    fig = plot_representative_paths(test_env, model, device, mc_df, run_policy_through_env_realdata)
    ...
    mkt_regime.to_csv(repo_root / "data" / "derived" / "market_regime.csv")
    report.save_dataframe(mkt_regime.to_frame(), "market_regime.csv")

    runs = []
    for i in range(1000):
        seed = 123 + i
        torch.manual_seed(seed)
        out = run_policy_through_env_realdata(complete_env, model, device=device, deterministic=False)
    ...
    agent_regime = build_agent_regime_outputs(out, df)
